<?php $__env->startSection('title', 'Deployment History & Rollback - Docker Control'); ?>
...
<div class="space-y-6" x-data="{ 
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 shadow-md">
            ...
            <div class="text-2xl font-bold text-white font-mono"><?php echo e($stats['total']); ?></div>
        ...
        <div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 shadow-md">
            ...
            <div class="text-2xl font-bold text-emerald-400 font-mono"><?php echo e($stats['successful']); ?></div>
            <div class="text-xs text-emerald-400/80 mt-1">
                <?php echo e($stats['total'] > 0 ? round(($stats['successful'] / $stats['total']) * 100) : 100); ?>% Success Rate
            </div>
        </div>
        ...
        <div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 shadow-md">
            ...
            <div class="text-2xl font-bold text-rose-400 font-mono"><?php echo e($stats['failed']); ?></div>
        ...
        <div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 shadow-md">
            ...
            <div class="text-2xl font-bold text-amber-400 font-mono"><?php echo e($stats['rollbacks']); ?></div>
        ...
        <div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 shadow-md col-span-2 md:col-span-1">
            ...
            <div class="text-2xl font-bold text-white font-mono"><?php echo e($stats['avg_build_time']); ?>s</div>
            <div class="text-xs text-sky-400 mt-1">Docker image push</div>
        </div>
    </div>

    <!-- Active Production Image Card -->
    <?php if($activeDeployment): ?>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="space-y-2">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs text-slate-400 font-mono">Environment: <?php echo e(strtoupper($activeDeployment->environment)); ?></span>
                    </div>

                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-200">
                        <span class="text-xl font-mono font-bold text-cyan-300"><?php echo e($activeDeployment->version); ?></span>
                        ...
                        <span class="text-sm font-mono text-slate-300 flex items-center gap-1">
                            ...
                            <?php echo e($activeDeployment->short_commit_id); ?>

                        </span>
                        <span class="text-slate-500">•</span>
                        <span class="text-sm text-slate-400 flex items-center gap-1">
                            <i data-lucide="git-branch" class="w-4 h-4 text-sky-400"></i>
                            <?php echo e($activeDeployment->branch); ?>

                        </span>
                        <span class="text-slate-500">•</span>
                        <span class="text-sm text-slate-400"><?php echo e($activeDeployment->commit_message); ?></span>
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <button @click="viewLog('<?php echo e($activeDeployment->version); ?>', '<?php echo e($activeDeployment->short_commit_id); ?>', `<?php echo e(addslashes($activeDeployment->deployment_logs)); ?>`)"
    ...
    <div class="rounded-2xl bg-slate-900/60 border border-slate-800 overflow-hidden shadow-lg">
        ...
        <div class="p-4 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/80">
            <div class="flex items-center gap-2">
                <form method="GET" action="<?php echo e(route('deployments.index')); ?>" class="flex items-center gap-2">
                    <div class="relative">
                        <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2"></i>
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search version, commit, deployer..." 
                    ...
                    <select name="status" onchange="this.form.submit()" class="px-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-xs text-slate-300 focus:outline-none focus:border-cyan-500">
                        ...
                        <option value="rolled_back" <?php echo e(request('status') === 'rolled_back' ? 'selected' : ''); ?>>Rolled Back</option>
            ...
            <div class="text-xs text-slate-400">
                Showing <?php echo e($deployments->firstItem() ?? 0); ?>-<?php echo e($deployments->lastItem() ?? 0); ?> of <?php echo e($deployments->total()); ?> deployments
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-slate-300">
                ...
                <tbody class="divide-y divide-slate-800/60 font-normal">
                    <?php $__empty_1 = true; $__currentLoopData = $deployments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-800/40 transition-colors <?php echo e($item->is_current_active ? 'bg-cyan-950/10' : ''); ?>">
                            
                            <!-- Version -->
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    <span class="font-mono font-bold text-sm text-cyan-300"><?php echo e($item->version); ?></span>
                                    <?php if($item->is_current_active): ?>
                                ...
                                <div class="text-[11px] text-slate-400 truncate max-w-xs mt-0.5"><?php echo e($item->commit_message); ?></div>
                            </td>
                            ...
                            <td class="px-5 py-4 font-mono">
                                <div class="text-slate-200 flex items-center gap-1">
                                    ...
                                    <?php echo e($item->short_commit_id); ?>
                            ...
                            <td class="px-5 py-4 font-mono text-slate-300">
                                <span class="flex items-center gap-1">
                                    ...
                                    <?php echo e($item->formatted_build_time); ?>
                            ...
                            <td class="px-5 py-4 text-slate-300">
                                <?php echo e($item->deployed_by); ?>

                            </td>

                            <!-- Timestamp -->
                            <td class="px-5 py-4 text-slate-400">
                                <div><?php echo e($item->deployed_at ? $item->deployed_at->format('d M Y, h:i A') : $item->created_at->format('d M Y, h:i A')); ?></div>
                                <div class="text-[10px] text-slate-500"><?php echo e($item->deployed_at ? $item->deployed_at->diffForHumans() : ''); ?></div>
                            </td>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <!-- View Logs Button -->
                                    <button @click="viewLog('<?php echo e($item->version); ?>', '<?php echo e($item->short_commit_id); ?>', `<?php echo e(addslashes($item->deployment_logs)); ?>`)" 
                                    ...
                                    <!-- Rollback Button -->
                                    <?php if(!$item->is_current_active && $item->status !== 'failed'): ?>
                                        <form method="POST" action="<?php echo e(route('deployments.rollback', $item->id)); ?>" onsubmit="return confirm('Are you sure you want to rollback to version <?php echo e($item->version); ?> (Commit <?php echo e($item->short_commit_id); ?>)? This will make this version live.')">
                                            <?php echo csrf_field(); ?>
    ...
    <div x-show="showDeployModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div @click.away="showDeployModal = false" class="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
            <form method="POST" action="<?php echo e(route('deployments.store')); ?>">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\git_desktop\PHP_Laravel12_Docker_Push_Image_In_DockerHUB\resources\views/deployments/index.blade.php ENDPATH**/ ?>