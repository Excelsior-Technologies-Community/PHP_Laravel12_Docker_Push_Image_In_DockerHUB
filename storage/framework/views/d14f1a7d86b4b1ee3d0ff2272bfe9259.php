<?php $__env->startSection('title', 'Log Viewer - Multi-Source Logs'); ?>
...
<div class="space-y-6" x-data="{
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <!-- Clear Log -->
            <form method="POST" action="<?php echo e(route('logs.clear')); ?>" onsubmit="return confirm('Are you sure you want to clear this log file? This action cannot be undone.')">
                <?php echo csrf_field(); ?>
            ...
            <!-- Download Log -->
            <a href="<?php echo e(route('logs.download', ['file' => $file ?: 'laravel.log'])); ?>" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition-colors flex items-center gap-1.5">
    ...
    <div class="flex flex-wrap items-center gap-2 border-b border-slate-800 pb-3">
        <a href="<?php echo e(route('logs.index', ['source' => 'laravel'])); ?>" 
           class="px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all <?php echo e($source === 'laravel' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-sm shadow-cyan-500/10' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'); ?>">
            <i data-lucide="layers" class="w-4 h-4 text-red-400"></i>
            <span>Laravel Logs (Framework)</span>
        </a>

        <a href="<?php echo e(route('logs.index', ['source' => 'docker'])); ?>" 
           class="px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all <?php echo e($source === 'docker' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-sm shadow-cyan-500/10' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'); ?>">
            <i data-lucide="container" class="w-4 h-4 text-blue-400"></i>
            <span>Docker Container Logs</span>
        </a>

        <a href="<?php echo e(route('logs.index', ['source' => 'nginx'])); ?>" 
           class="px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all <?php echo e($source === 'nginx' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-sm shadow-cyan-500/10' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'); ?>">
            <i data-lucide="globe" class="w-4 h-4 text-emerald-400"></i>
            <span>Nginx Server Logs</span>
        </a>

        <a href="<?php echo e(route('logs.index', ['source' => 'php'])); ?>" 
           class="px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all <?php echo e($source === 'php' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 shadow-sm shadow-cyan-500/10' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'); ?>">
    ...
    <div class="flex flex-wrap items-center gap-2">
        ...
        <?php $__currentLoopData = $levels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lvlKey => $lvlInfo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a href="<?php echo e(route('logs.index', array_merge(request()->query(), ['level' => $lvlKey]))); ?>" 
               class="px-3 py-1.5 rounded-lg text-xs font-medium border flex items-center gap-2 transition-all <?php echo e($level === $lvlKey ? 'ring-2 ring-cyan-500 ' . $lvlInfo['color'] : 'bg-slate-900/60 text-slate-400 border-slate-800 hover:text-slate-200'); ?>">
                <span><?php echo e($lvlInfo['label']); ?></span>
                <span class="px-1.5 py-0.2 rounded-full text-[10px] font-mono bg-black/40 text-slate-300">
                    <?php echo e($levelCounts[$lvlKey] ?? 0); ?>
    ...
    <div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 shadow-md">
        <form method="GET" action="<?php echo e(route('logs.index')); ?>" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <input type="hidden" name="source" value="<?php echo e($source); ?>">
            ...
            <div>
                ...
                <select name="file" onchange="this.form.submit()" class="w-full px-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-cyan-500">
                    <?php $__currentLoopData = $availableFiles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $f): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($f['name']); ?>" <?php echo e(($file === $f['name'] || (!$file && $loop->first)) ? 'selected' : ''); ?>>
                            <?php echo e($f['name']); ?> (<?php echo e($f['size']); ?>)
                        </option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Search messages, trace, error code..." 
            ...
            <div class="flex items-end gap-2">
                <div class="flex-1">
                    ...
                    <select name="limit" class="w-full px-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-cyan-500">
                        <option value="50" <?php echo e(request('limit') == 50 ? 'selected' : ''); ?>>50 lines</option>
                        <option value="100" <?php echo e(request('limit', 100) == 100 ? 'selected' : ''); ?>>100 lines</option>
                        <option value="250" <?php echo e(request('limit') == 250 ? 'selected' : ''); ?>>250 lines</option>
                        <option value="500" <?php echo e(request('limit') == 500 ? 'selected' : ''); ?>>500 lines</option>
                    </select>
                </div>
    ...
    <div class="rounded-2xl bg-slate-900/60 border border-slate-800 overflow-hidden shadow-lg">
        ...
        <div class="p-3 border-b border-slate-800 bg-slate-900/80 flex items-center justify-between text-xs text-slate-400">
            <span class="flex items-center gap-1.5">
                ...
                Log Source: <strong class="text-slate-200 font-mono"><?php echo e(strtoupper($source)); ?></strong> • File: <strong class="text-cyan-300 font-mono"><?php echo e($logData['file']); ?></strong>
            </span>
            <span>Showing <?php echo e(count($logData['entries'])); ?> of <?php echo e($logData['total']); ?> matching entries</span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs font-mono">
                ...
                <tbody class="divide-y divide-slate-800/60">
                    <?php $__empty_1 = true; $__currentLoopData = $logData['entries']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-800/40 transition-colors">
                            ...
                            <td class="px-4 py-3 align-top">
                                ...
                                ?>
                                <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold border <?php echo e($badgeColor); ?>">
                                    <?php echo e($lvl); ?>
                            ...
                            <td class="px-4 py-3 text-slate-200 break-words align-top">
                                <div class="line-clamp-2"><?php echo e($entry['message']); ?></div>
                            ...
                            <td class="px-4 py-3 text-right align-top">
                                <button @click="viewDetail(<?php echo e(json_encode($entry)); ?>)" 
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\git_desktop\PHP_Laravel12_Docker_Push_Image_In_DockerHUB\resources\views/logs/index.blade.php ENDPATH**/ ?>