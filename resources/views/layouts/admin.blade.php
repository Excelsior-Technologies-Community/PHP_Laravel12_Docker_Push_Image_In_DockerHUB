...
<head>
    ...
    <title>@yield('title', 'Laravel 12 Docker Control')</title>
    <!-- Tailwind CSS CDN -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <a href="{{ route('system.health') }}" class="text-lg font-bold bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-500 bg-clip-text text-transparent">
                ...
                <nav class="flex items-center space-x-1 sm:space-x-2">
                    <a href="{{ route('deployments.index') }}" 
                       class="px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-150 flex items-center gap-2 {{ request()->routeIs('deployments.*') ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60' }}">
                        <i data-lucide="history" class="w-4 h-4"></i>
                        <span>Deployments</span>
                    </a>

                    <a href="{{ route('system.health') }}" 
                       class="px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-150 flex items-center gap-2 {{ request()->routeIs('system.health*') ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60' }}">
                        <i data-lucide="activity" class="w-4 h-4"></i>
                        <span>Live Health</span>
                    </a>

                    <a href="{{ route('logs.index') }}" 
                       class="px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-150 flex items-center gap-2 {{ request()->routeIs('logs.*') ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60' }}">
